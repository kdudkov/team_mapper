...
def distance(p1, p2):
    ...
    x = math.sqrt(pow((earth_point_1_x - earth_point_2_x), 2) + pow((
        earth_point_1_y - earth_point_2_y), 2))
    y = math.pi * ((earth_point_1_x + earth_point_2_x) / 360.) * (
        p1.lon - p2.lon)
    ...
    fAlpha = math.asin(
        math.cos(math.radians(p2.lat)) * math.sin(fdLambda) / math.sin(fz))
...
if __name__ == '__main__':
    lon, lat = 30.4, 60.0
    d1 = distance(Point(lon, lat), Point(lon + 0.5, lat))[0] / 0.5
    print "lon: %.2f m/deg" % d1
    d2 = distance(Point(lon, lat), Point(lon, lat + 0.5))[0] / 0.5
    print "lat: %.2f m/deg" % d2
